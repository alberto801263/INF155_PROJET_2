...
#define PEN_CACHE_SIZE   11
#define FONT_CACHE_SIZE  11
#define BG               16
#define TIMER_ID         1
...
static struct { int width; int height; } font_metrics[][11] = {
    ...
    {{0,0},{13,16},{14,20},{16,23},{22,31},{29,41},{36,51},{44,62},{55,77},{66,93},{88,124}}, // TriplexFont
...
// I am not sure about metrics of following fonts
    ...
    {{0,0},{13,16},{14,20},{16,23},{22,31},{29,41},{36,51},{44,62},{55,77},{66,93},{88,124}}, // TriplexScriptFont
...
void settextsize(int xsize, int ysize) {
    ...
    font_metrics[0][1].width = xsize;
    font_metrics[0][1].height = ysize;
...
static void text_output(int x, int y, const char* str)
{
    ...
    if (bgiemu_handle_redraw || visual_page != active_page)
   		TextOut(hdc[1], x, y, str, (size_t)strlen(str));
...
static void initwork(int height, int width, int* device, int* mode)
{
    ...
    // Quick fix sinon la taille du texte est fixée
    // selon la taille de l'écran par Windows (OP E2017).
    settextsize(8,14);
